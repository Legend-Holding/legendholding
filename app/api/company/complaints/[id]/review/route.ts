...
export async function POST(
  ...
  try {
    ...
    await query(
      `UPDATE customer_care_complaints SET status = 'reviewed', reviewed_at = $1 WHERE id = $2`,
      [new Date().toISOString(), id],
    );

    return NextResponse.json({ success: true, message: 'Complaint marked as reviewed' }, { status: 200 });
  } catch (error: unknown) {
